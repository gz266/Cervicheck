 ...
 * @return Success or failure
 */
bool AD5933::calibrate(double gain[], double phaseRef[], int ref, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        phaseRef[i] = atan2(imag[i], real[i]) * (180.0 / M_PI); // Convert to degrees
    }
 ...
 * @return Success or failure
 */
bool AD5933::calibrate(double gain[], double phaseRef[], int real[], int imag[],
                       int ref, int n) {
    // Perform the frequency sweep
    if (!frequencySweep(real, imag, n)) {
        return false;
